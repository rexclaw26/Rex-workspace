// app/api/headline-xpost/route.ts
// Direct execution endpoint for X post rewrites from Headlines page.
// Receives headline data → calls OpenRouter LLM → sends 3 versions to Kelly's Telegram.
// No task queue — executes inline, same pattern as /api/x-rewrite.
...
async function callLLM(prompt: string): Promise<string> {
  const res = await fetch("https://openrouter.ai/api/v1/chat/completions", {
    ...
    headers: {
      ...
      "X-Title": "Mission Control - Headline X Post",
    },
    body: JSON.stringify({
      model: MODEL,
      max_tokens: 600,
...
// ── Prompt ────────────────────────────────────────────────────────────────────
function buildPrompt(hl: HeadlinePayload): string {
  const pubDate = new Date(hl.pubDate).toLocaleDateString("en-US", {
    month: "short", day: "numeric", year: "numeric",
  });

  return `You are rewriting a news headline as @DiscoverCrypto on X (1M+ followers). This account covers crypto, macro, DeFi, tokenization, and institutional finance.
...
SOURCE HEADLINE: ${hl.title}
SOURCE: ${hl.source} | PUBLISHED: ${pubDate}
SUMMARY: ${hl.description ?? "(no summary available)"}
CATEGORY: ${hl.category} | SENTIMENT: ${hl.sentiment}
URL: ${hl.url}
...
- Only facts present in the headline/summary — no fabrication
...
function formatForTelegram(hl: HeadlinePayload, llmOutput: string): string {
  ...
  const source  = `${hl.source} · ${new Date(hl.pubDate).toLocaleDateString("en-US", { month: "short", day: "numeric" })}`;
  ...
  return [
    ...
    cleaned,
    ``,
    `<a href="${hl.url}">Read full story ↗</a>`,
  ].join("\n");
}
...
export async function POST(request: Request) {
  ...
  let headline: HeadlinePayload;
  try {
    const body = await request.json();
    headline = body.headline;
    if (!headline?.title) throw new Error("Missing headline data");
  } catch {
  ...
  try {
    ...
    const message = formatForTelegram(headline, llmOut);
    await sendTelegram(message);

    return NextResponse.json({ ok: true });
  } catch (err: any) {
    console.error("[headline-xpost]", err.message);
