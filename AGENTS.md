...
**PR-047 — Sub-Agent and Plan Execution Timeouts (2026-03-25, permanent):** When planning any research, build, or fix plan: (1) chunk to fit 120s max per agent pass — split if more than 5 things to do. (2) Write partial results immediately upon completion, never only at end — if timeout fires, the file still has what completed. (3) Continuation protocol — next agent reads partial file, does ONLY the missing parts. (4) Critic gets pre-written summary to critique, never research AND critique in same pass.
