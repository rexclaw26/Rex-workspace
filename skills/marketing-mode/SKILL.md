...
## ⚠️ Hit Network Hierarchy Rule (always active)

This skill is the **strategic layer only**. It selects the right playbook and sets direction. Execution always routes to the appropriate specialist skill — which runs its own full quality gates, approval requirements, and procedures.

| This skill handles | Routes to specialist skill |
|-------------------|--------------------------|
| SEO strategy selection | `seo-audit` (execution + technical audit) |
| Copy strategy + messaging | `copywriting` (framework + LAW 1 gate) |
| Social content direction | `x-post-automator` (drafting + Kelly approval) |
| Content calendar strategy | `content-strategy` (data-driven calendar) |
| Campaign ROI framing | `financial-analysis` (sourced figures) |

**This skill NEVER overrides the procedures, quality gates, or human approval requirements of specialist skills. It orchestrates. They execute.**
